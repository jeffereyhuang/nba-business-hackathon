...
\textit{X} - Index variable
\textit{Followers} - # of Followers at time of posting
\textit{Created} - Timestamp of posting
\textit{Type} - Type of Post; three factors: \textit{Videos, Albums, Photos}
\textit{Description} - Post caption
\textit{datetime} - Datetime variable parsed from \textit{Created}
\textit{year} - Year it was posted
\textit{month} - Month posted (parsed \textit{datetime})
\textit{month_int} - Numeric variable for months (\textit{1} = January, \textit{12} = December);
                     Useful for correlation plot
\textit{weekday} - Day of week posted
\textit{day_time} - Time of day posted
\textit{is_playoffs} - Boolean: whether the post was made in the playoffs (True) or not (False)
\textit{is_season} - Boolean: whether the post was made during the season/playoffs (True) or off-season (False).
\textit{Followers Mentioned} - Number of followers of nba team accounts mentioned in post. \textit{Description}
\textit{all_nba} - Three factor variable: \textit{0} if no all-nba or all-star players are mentioned in \textit{Description}. \textit{1} if an all-nba is mentioned in the \textit{Description}, not including those to follow. \textit{2} if Lebron James, Stephen Curry, Kyrie Irving, or Kobe Bryant (NBA players with highest number of instagram followers) are mentioned. Although Kobe is retired, posts with his mention appeared to garner more engagements.
\textit{league_type} - \textit{1} if \textit{Description} mentioned the WNBA or G-League \textit{1} otherwise.
\textit{same_day_post} - \textit{1} if post was created within the same \textit{day_time} on the same day. \textit{0} otherwise. Parsed from \textit{Created}.
\textit{num_ats} - Number of hashtags in the \textit{Description}.
\textit{num_hash} - Number of @ symbols in the \textit{Description}.
\textit{Type_num} - Numerical variable for \textit{Type} (\textit{1} = Video, \textit{2} = Album, \textit{3} = Photo);
                    Useful for correlation plot
\textit{wkdy_num} - Only in training. Numerical variable for \textit{weekday} (\textit{1} = Monday, \textit{7} = Sunday)
\textit{day_time_num} - Only in training. Numerical variable for \textit{day_time} (\textit{1} = deazone, \textit{7} = postgame)
...
1) The correlation between \textit{team_foll_ment} and \textit{num_@}. Intuitively this makes sense, a post must mention another team's IG account in order for the \textit{team_foll_ment} to be a non-zero number. It may be worth looking into removing this variable after some cross validation.

2) The correlation between \textit{day_time_num} and \textit{same_day_post}. Intuitively this makes sense, a post must be, by our definition, during the same time period as another for \textit{same_day_post} to return a 1. It may be worth looking into removing one of these variables after some cross validation.
...
More extensive cross-validation methods were used as well, but for brevity's sake, we don't include them here. Our cross-validation was useful in selecting the correct \textit{mtry}, as well as eliminating two predictors that appeared to aid the model little: \textit{same_day_post}, and \textit{is_playoffs}. After eliminating these variables and tuning, our model's improved by over 1%.

```{r}
# Creating Video/Photo/Album subsets from nba data

#nba$Followers = as.numeric(nba$Followers)
#nba$month = as.factor(nba$month)
#nba$day_time = as.factor(nba$day_time)
#nba$weekday = as.factor(nba$weekday)
#nba$Followers_Mentioned = as.numeric(nba$Followers_Mentioned)
#nba$is_season = as.factor(nba$is_season)
...
nba_Photo = nba[nba$Type_num == 2,]
nba_Album = nba[nba$Type_num == 3,]
...
RF_Album <- randomForest(Engagements ~ Followers+month+day_time+weekday+Followers_Mentioned+is_season+all_nba+league_type+num_ats+num_hash, ntree=1000, mtry=8,importance=TRUE, data=train_dataRF_Album)
RF_Album

RF_Photo <- randomForest(Engagements ~ Followers+month+day_time+weekday+Followers_Mentioned+is_season+all_nba+league_type+num_ats+num_hash, ntree=1000, mtry=7,importance=TRUE, data=train_dataRF_Photo)
RF_Photo
...
```{r}
# Add num_ats, num_hash variables and remove season_yr and is_playoffs column 
```

```{r}
## Final models for training and prediction

RF_Video_Final <- randomForest(Engagements ~ Followers+month+day_time+weekday+Followers_Mentioned+is_season+all_nba+league_type+num_ats+num_hash, ntree=1000, mtry=7, maxnodes=100,importance=TRUE, data=nba_Video)

RF_Photo_Final <- randomForest(Engagements ~ Followers+month+day_time+weekday+Followers_Mentioned+is_season+all_nba+league_type+num_ats+num_hash, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=nba_Photo)

RF_Album_Final <- randomForest(Engagements ~ Followers+month+day_time+weekday+Followers_Mentioned+is_season+all_nba+league_type+num_ats+num_hash, ntree=1000, mtry=7,importance=TRUE, data=nba_Album)
```
...
holdout_video[Engagements] = predict(RF_Video_Final, holdout_video)
holdout_photo[Engagements] = predict(RF_Photo_Final, holdout_photo)
holdout_album[Engagements] = predict(RF_Album_Final, holdout_album)
