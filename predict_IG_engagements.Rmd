...
g <- ggplot(nba_graphs, aes(x = Type, y = Engagements, fill = season_yr))
g + geom_boxplot(position = "dodge", outlier.shape=NA) + labs(title="Effect of Season Year on Engagements", x= "Season Year", y="Engagements") + theme_minimal()
...
nba_corr$season_yr = as.numeric(nba$season_yr)
...
# 2,9,10,11,12,14,15,17:22
# Create a plot correlation matrix
nbavars <- nba_corr[ ,c(2,9,12,14,15,17:22,24,25)]
res <- cor(nbavars)
round(res, 2)
...
Mean number of Video posts: `r print(sum(nba$Type=='Video'))`
Mean number of Photo posts: `r print(sum(nba$Type=='Album'))`
Mean number of Album posts: `r print(sum(nba$Type=='Photo'))`
...
numberOfTrainingSamples_Video <- round(length(nba_Video$Engagements) * .75)
numberOfTrainingSamples_Photo <- round(length(nba_Photo$Engagements) * .75)
numberOfTrainingSamples_Album <- round(length(nba_Album$Engagements) * .75)
...
RF_Video <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, xtest = test_dataRF_Video[,c(2,8,10,11,12,15,17:22)], ytest = test_dataRF_Video$Engagements, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=train_dataRF_Video)
RF_Video

RF_Photo <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, xtest = test_dataRF_Photo[,c(2,8,10,11,12,15,17:22)], ytest = test_dataRF_Photo$Engagements, ntree=1000, mtry=9, maxnodes=100,importance=TRUE, data=train_dataRF_Photo)
RF_Photo

RF_Album <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, xtest = test_dataRF_Album[,c(2,8,10,11,12,15,17:22)], ytest = test_dataRF_Album$Engagements, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=train_dataRF_Album)
RF_Album
```

```{r echo=FALSE}
# Calculating MAPE of training sets
RF_Video_for_calc <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=train_dataRF_Video)

RF_Photo_for_calc <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=7, maxnodes=100,importance=TRUE, data=train_dataRF_Photo)

RF_Album_for_calc <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=7, maxnodes=100,importance=TRUE, data=train_dataRF_Album)
...
mape_Rf
```
Our validation MAPE came out to about 5.5%. Not bad!
...
```{r}
# Add num_ats, num_hash, Type_num, wkday_num, day_time_num variables
...
for(i in 1:length(holdout[,1])){
  if(is.na(holdout$season_yr[i]))(holdout$season_yr[i] = 0)
}

holdout$Type_num <- NA
for(i in 1:length(holdout[,1])){
  if(holdout$Type[i] == "Video")(holdout$Type_num[i] = 1)
  if(holdout$Type[i] == "Photo")(holdout$Type_num[i] = 2)
  if(holdout$Type[i] == "Album")(holdout$Type_num[i] = 3)
}
holdout$wkdy_num <- NA
for(i in 1:length(holdout[,1])){
  if(holdout$weekday[i] == "Sunday")(holdout$wkdy_num[i] = 1)
  if(holdout$weekday[i] == "Monday")(holdout$wkdy_num[i] = 2)
  if(holdout$weekday[i] == "Tuesday")(holdout$wkdy_num[i] = 3)
  if(holdout$weekday[i] == "Wednesday")(holdout$wkdy_num[i] = 4)
  if(holdout$weekday[i] == "Thursday")(holdout$wkdy_num[i] = 5)
  if(holdout$weekday[i] == "Friday")(holdout$wkdy_num[i] = 6)
  if(holdout$weekday[i] == "Saturday")(holdout$wkdy_num[i] = 7)
}
holdout$day_time_num <- NA
for(i in 1:length(holdout[,1])){
  if(holdout$day_time[i] == "deadzone")(holdout$day_time_num[i] = 1)
  if(holdout$day_time[i] == "morning")(holdout$day_time_num[i] = 2)
  if(holdout$day_time[i] == "lunch")(holdout$day_time_num[i] = 3)
  if(holdout$day_time[i] == "afternoon")(holdout$day_time_num[i] = 4)
  if(holdout$day_time[i] == "evening")(holdout$day_time_num[i] = 5)
  if(holdout$day_time[i] == "night")(holdout$day_time_num[i] = 6)
  if(holdout$day_time[i] == "postgame")(holdout$day_time_num[i] = 7)
}
```
...
RF_Video_Final <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+season_yr+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=nba_Video)

RF_Photo_Final <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+season_yr+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=9, maxnodes=100,importance=TRUE, data=nba_Photo)

RF_Album_Final <- randomForest(Engagements ~ Followers+month+weekday+day_time+Followers_Mentioned+is_playoffs+season_yr+is_season+all_nba+league_type+same_day_post+num_ats+num_hash, ntree=1000, mtry=8, maxnodes=100,importance=TRUE, data=nba_Album)
```
